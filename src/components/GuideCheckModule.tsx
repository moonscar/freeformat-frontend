...
export default function GuideCheckModule({
  ...
  React.useEffect(() => {
    ...
    const applyHash = () => {
      if (window.location.hash === '#format-check') setOpen(true);
    };
    applyHash();
  ...
  return (
    <section id="format-check" className="rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-sm">
      <details open={open} onToggle={(e) => setOpen((e.target as HTMLDetailsElement).open)}>
        <summary className="cursor-pointer list-none">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="mt-1 shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-700">
